...
document.getElementById("convntusd").addEventListener("click",function(){
    
  //  console.log("running");
    
    //check if value to be converted is 0
    if(!document.getElementById("r1ngn").value == 0){

var firstinput = document.getElementById("r1ngn");
var resultinput = document.getElementById("resultusd");
...
//get rate from LS
var ngrate = localStorage.getItem("ntus");

//remove coma
var newngrate = ngrate.replace(/,(?=\d{3})/g, '')

//calculatev
var finalvalue = parseFloat(newngrate) * from;


//display result
resultinput.innerHTML = finalvalue;
...
if(localStorage.getItem("all_data_ntd")){
    
    console.log("data found");
    //get it and check for timestamp
    var p_data = JSON.parse(localStorage.getItem("all_data_ntd"));
 ...
 function loadApi()
        ...
        //define urls
        var url1 = "https://api.exchangerate.host/"+today6+"?base=USD&symbols=NGN,USD&amount=1";
        var url2 = "https://api.exchangerate.host/"+today5+"?base=USD&symbols=NGN,USD&amount=1";
        var url3 = "https://api.exchangerate.host/"+today4+"?base=USD&symbols=NGN,USD&amount=1";
        var url4 = "https://api.exchangerate.host/"+today3+"?base=USD&symbols=NGN,USD&amount=1";
        var url5 = "https://api.exchangerate.host/"+today2+"?base=USD&symbols=NGN,USD&amount=1";
        var url6 = "https://api.exchangerate.host/"+today1+"?base=USD&symbols=NGN,USD&amount=1";
   ...
   //get timestamp
    ...
    all_data_ntd.push(addtime);
   
   //save data in ls
   localStorage.setItem("all_data_ntd", JSON.stringify(all_data_ntd));
    
    
    //display main rate
     document.getElementById("conn").innerHTML = data[6].result;
     
     //save in LS
     localStorage.setItem("ntus", data[6].result);
...
const myChart = new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [{
            label: '1 Naira to Dollars exchange rate in the last 6 months',
  ...
  //display in dable
   document.getElementById("mm1").innerHTML = m6;
    ...
    function loadLS(){
        ...
        //get the data
        data = JSON.parse(localStorage.getItem("all_data_ntd"));
    ...
    //display main rate
     ...
     if(!localStorage.setItem("ntus", data[6].result))
     {
        //save in LS
     localStorage.setItem("ntus", data[6].result);
     }
...
const myChart = new Chart(ctx, {
    ...
    data: {
        labels: [m6, m5, m4, m3, m4, m1],
        datasets: [{
            label: '1 Naira to Dollar exchange rate in the last 6 months',
            data: [data[0].rates.NGN, data[1].rates.NGN, data[2].rates.NGN, data[3].rates.NGN, data[4].rates.NGN, data[5].rates.NGN],
            backgroundColor: [
                'rgba(255, 99, 132, 0.2)',
                'rgba(54, 162, 235, 0.2)',
